...
# make sure this runs: opens a file dialogue that can open an image in a new window
...
id = signal_connect(b, "clicked") do widget
    ## open a file dialogue (how?)
    filename = open_dialog("Pick an image")
    println(filename)
    ## load file and push image array to img
    setupfile(filename)
end;
